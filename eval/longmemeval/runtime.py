...
class BenchmarkRuntime:
    ...
    consolidation: object # BenchmarkConsolidationAdapter
    workspace: Path


class BenchmarkConsolidationAdapter:
    """Compatibility wrapper for the benchmark's older consolidation calls."""
    ...
    async def consolidate(
        ...
        *,
        ...
        return await self._maintenance.consolidate(
            ConsolidateRequest(
                session=session,
...
async def create_runtime(config_path: Path, workspace: Path) -> BenchmarkRuntime:
    ...
    # 4. Reuse the production markdown memory maintenance object, but expose
    #    the old benchmark call shape: consolidate(session, archive_all=...).
