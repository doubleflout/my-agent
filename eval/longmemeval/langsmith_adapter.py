...
benchmark QA calls when the eval CLI explicitly asks for it.
=======
This module keeps LangSmith out of the production agent path. It only wraps
...
<<<<<<< HEAD
    ...
    The traced function accepts only serialisable benchmark inputs.  The actual
    runtime and instance objects stay in the closure so LangSmith does not try to
    upload the full Agent runtime object graph.
    """
