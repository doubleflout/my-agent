...
def _ensure_postgres_trace_identity(conn: Any) -> None:
    for table in ("turns", "rag_queries", "memory_writes"):
        row = conn.execute(
            """
        ...
        if row is None:
            continue
        is_identity = str(row[0] or "").upper() == "YES"
        has_default = row[1] is not None
